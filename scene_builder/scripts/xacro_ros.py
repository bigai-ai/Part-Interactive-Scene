...
class XacroROS(object):
    ...
    def add_object(self, link_name, mesh_dir, xyz, rpy, 
        scale=[1.0, 1.0, 1.0], mass=1, inertia=[1, 0, 0, 1, 0, 1], color=None, add_collision=False, ns=None
    ):
        """
        ...
        self.add_mesh_to_element_(visual, mesh_dir, xyz, rpy, scale, color)
    ...
    def add_joint(self, joint_name, joint_type, child_link, parent_link, 
        xyz, rpy, joint_axis=None, joint_friction=None, joint_effort=None,
        joint_velocity=None, joint_lower=None, joint_upper=None, 
        add_gazebo_tag=False, ns=None
    ):
        """
        ...
        @param ns (string): the namespace of the element
        """
        parent = self.get_parent_element_(ns)
        ...
        if joint_axis is not None:
            axis_link_element = ET.SubElement(joint, "axis")
            axis_link_element.set("xyz", "{} {} {}".format(*joint_axis))

        if joint_friction is not None:
            dynamics_link_element = ET.SubElement(joint, "dynamics")
            dynamics_link_element.set("friction", "{}".format(joint_friction))
        
        if joint_effort is not None:
            assert(joint_velocity is not None)
            assert(joint_lower is not None)
            assert(joint_upper is not None)

            limit_link_element = ET.SubElement(joint, "limit")
            limit_link_element.set("effort", "{}".format(joint_effort))
            limit_link_element.set("velocity", "{}".format(joint_velocity))
            limit_link_element.set("lower", "{}".format(joint_lower))
            limit_link_element.set("upper", "{}".format(joint_upper))
    ...
    def add_mesh_to_element_(self, element, mesh_dir, xyz, rpy, scale=[1.0, 1.0, 1.0], color=None):
        """
        ...
        if color is not None:
            ...
            material.set("name", "color_{}_{}_{}".format(*[int(c * 255) for c in color[:3]]))

            color_ele = ET.SubElement(material, "color")
            color_ele.set("rgba", "{} {} {} {}".format(*color))
